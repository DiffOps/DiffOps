...
4. **Git & Issues**: toda unidade nasce de uma **issue no GitHub** (Problemática → Causa/contexto → Escopo → Critérios de aceitação) e só então ganha branch `@<user>/<issue>/<tipo>/<descricao>`; a PR inclui `Closes #<issue>` e o merge do usuário a fecha. Conventional Commits **atômicos** (1 unidade lógica = 1 commit, suíte verde).
