...
it('renders empty analytics instead of erroring when the user has no organization', function () {
    User::create([
        'name' => 'Operator',
    ...
    $this->withUnencryptedCookie('diffops_session', TestJwtSigner::sign())
        ...
        ->assertInertia(fn ($page) => $page
            ...
            ->where('verdictDistribution', [])
            ->where('threatHistogram', [])
            ->where('defconTrend', [])
            ->where('findingsByCategory', [])
            ->where('topRepos', []));
});
